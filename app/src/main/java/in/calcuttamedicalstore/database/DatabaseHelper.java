package in.calcuttamedicalstore.database;
...
import static in.calcuttamedicalstore.activity.HomeActivity.txt_countcard;
...
public class DatabaseHelper extends SQLiteOpenHelper {
  ...
  public void onCreate(SQLiteDatabase db) {
    db.execSQL(
        "create table "
            + TABLE_NAME
            + " (ID INTEGER PRIMARY KEY AUTOINCREMENT, PID TEXT , image TEXT ,title TEXT , weight TEXT , cost TEXT, qty TEXT , discount int )");
  }
  ...
  @SuppressLint("SetTextI18n")
  public void deleteRData(String id, String cost) {
    SQLiteDatabase db = this.getWritableDatabase();
    db.delete(TABLE_NAME, "PID = ? AND cost =?", new String[] {id, cost});
    Cursor res = getAllData();
    txt_countcard.setText("" + res.getCount());
  }
}
